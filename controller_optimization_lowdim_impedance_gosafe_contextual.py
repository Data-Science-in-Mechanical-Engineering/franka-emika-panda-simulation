...
class GoSafe_Optimizer(object):
    ...
    def __init__(self, upper_overshoot=0.08,upper_eigenvalue=-10, lengthscale=0.3, ARD=True):
        self.upper_eigenvalue=upper_eigenvalue
        self.upper_overshoot=upper_overshoot
        q =4/6
        r = -1
        ...
        L = [lengthscale / 6, lengthscale / 3, 0.35, 0.33, 0.35, 1.5, 1.5,1.5]
        ...
        self.opt = GoSafeSwarm_Contextual(gp=[gp0, gp1], gp_full=[gp_full0, gp_full1], bounds=bounds,
                                     fmin=[-np.inf, 0], x_0=x0.reshape(-1,1),L_states=L_states ,eta_L=0.25, max_S1_steps=60,
                                     max_S3_steps=10, eps=0.1, max_data_size=1000, reset_size=500,boundary_thresshold_u=0.88)
        self.opt.fast_safe_action=False
        #self.opt.boundary_thresshold=0.9
...
if method=="GoSafe":
    ...
    for r in range(runs):
        ...
        random.seed((runs-1-r))
        np.random.seed((runs-1-r))
        opt.sys.env.seed((runs-1-r))
        opt.opt._seed((runs-1-r))
        if r>0:
            plot=False
